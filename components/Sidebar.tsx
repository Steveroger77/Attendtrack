...
const Sidebar: React.FC<SidebarProps> = ({ sidebarOpen, setSidebarOpen, page, setPage, pendingLeaveCount }) => {
  ...
  return (
    ...
    <div
        ref={sidebar}
        className={`flex flex-col absolute z-40 left-0 top-0 lg:static lg:left-auto lg:top-auto lg:translate-x-0 transform h-screen overflow-y-auto overflow-x-hidden no-scrollbar w-64 lg:w-20 lg:hover:w-64 shrink-0 bg-black/20 backdrop-blur-2xl border-r border-white/10 p-4 transition-[width] duration-300 ease-in-out group ${ sidebarOpen ? 'translate-x-0' : '-translate-x-64'}`}
    >
        <div className="flex justify-between mb-10 pr-3 sm:px-2 items-center overflow-hidden h-6">
            <div className="transition-opacity duration-200 ease-in-out delay-150 lg:opacity-0 lg:group-hover:opacity-100 flex items-center min-w-max shrink-0">
