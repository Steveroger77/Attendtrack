...
class MockApi {
  ...
  getRequiredAttendancePercentage(): number {
    const settings = db.load(DB_KEYS.SETTINGS, initialSettings);
    return settings.required_attendance_percentage ?? 75;
  }

  getWarningAttendancePercentage(): number {
    const settings = db.load(DB_KEYS.SETTINGS, initialSettings);
    return settings.warning_attendance_percentage ?? 80;
  }
  ...
  async updateLecturerAssignments(lecturerId: number, sectionIds: number[]): Promise<void> {
    ...
    async generateSessionPin(timetableId: number): Promise<string> {
        ...
        const activeSessions = db.load<any[]>('db_active_sessions', []);
        
        const filtered = activeSessions.filter(s => s.timetableId !== timetableId);
        filtered.push({
            timetableId,
            pin,
        ...
        db.save('db_active_sessions', filtered);
        return pin;
    }

    async getActivePinForTimetable(timetableId: number): Promise<string | null> {
        const activeSessions = db.load<any[]>('db_active_sessions', []);
        const session = activeSessions.find(s => s.timetableId === timetableId && s.expiresAt > Date.now());
    ...
    async studentCheckIn(studentId: number, pin: string): Promise<{ success: boolean; message: string; courseTitle?: string }> {
        ...
        if (!session) {
            return { success: false, message: "Invalid or expired check-in PIN." };
        }

        const timetable = db.load<TimetableEntry[]>(DB_KEYS.TIMETABLE, []);
        const entry = timetable.find(t => t.id === session.timetableId);
        if (!entry) {
            return { success: false, message: "Associated class period not found." };
        }

        const enrollments = db.load<Enrollment[]>(DB_KEYS.ENROLLMENTS, []);
        const studentEnrollment = enrollments.find(e => e.student_id === studentId && e.section_id === entry.section_id);
        if (!studentEnrollment) {
            return { success: false, message: `You are not enrolled in Sec ${entry.section.section_name} of ${entry.course.title}.` };
        }

        const date = new Date().toISOString().split('T')[0];
        const attendanceRecords = db.load<AttendanceRecord[]>(DB_KEYS.ATTENDANCE_RECORDS, []);
        const attendanceAudits = db.load<AttendanceAudit[]>(DB_KEYS.ATTENDANCE_AUDITS, []);
        let nextIds = db.load<any>(DB_KEYS.NEXT_IDS, initialNextIds);

        const existingRecord = attendanceRecords.find(r => r.enrollment_id === studentEnrollment.id && r.date === date && r.period_index === entry.period_index);
        
        if (existingRecord) {
            if (existingRecord.status === AttendanceStatus.PRESENT) {
                return { success: true, message: "You are already checked in!", courseTitle: entry.course.title };
            }
            const oldStatus = existingRecord.status;
            existingRecord.status = AttendanceStatus.PRESENT;
            existingRecord.marked_by = entry.lecturer_id;
            existingRecord.marked_at = new Date().toISOString();
            existingRecord.version += 1;
...
class ApiService {
  ...
  getRequiredAttendancePercentage = () => this.api.getRequiredAttendancePercentage();
  getWarningAttendancePercentage = () => this.api.getWarningAttendancePercentage();
  ...
  // Active PIN check-in sessions
  generateSessionPin = (timetableId: number) => this.withStatelessDB(api => api.generateSessionPin(timetableId));
  getActivePinForTimetable = (timetableId: number) => this.withStatelessDB(api => api.getActivePinForTimetable(timetableId));
  studentCheckIn = (studentId: number, pin: string) => this.withStatelessDB(api => api.studentCheckIn(studentId, pin));
}
